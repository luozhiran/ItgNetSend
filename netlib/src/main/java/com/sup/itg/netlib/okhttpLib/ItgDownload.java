    ...
    public ItgDownload registerCallback() {
        this.task().itgProgressBack(new ItgProgressback() {
            ...
            public void itgProgress(ItgTask task) {
                ...
                if (task.getProgress() == 100 && (task.broadcast() || !TextUtils.isEmpty(task.customBroadcast()))) {
                    Intent intent = null;
                    if (task.broadcast()) {
                        intent = new Intent(ItgNetSend.BROAD_ACTION);
                    } else {
                        if (!TextUtils.isEmpty(task.customBroadcast())) {
                            intent = new Intent(task.customBroadcast());
                        }
                    }
